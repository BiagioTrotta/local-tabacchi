<x-layout.app :title="$title">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            /* ==========================
               FUNZIONE UNICA DI INSERIMENTO
            ========================== */
            function addItemToList(item) {

                if (!item.ean) {
                    alert('EAN mancante: impossibile aggiungere il prodotto');
                    return;
                }

                if (!scannedItems[item.ean]) {
                    scannedItems[item.ean] = {
                        ean: item.ean,
                        codice: item.codice,
                    ...
                    scannedItems[item.ean].quantity++;
                }
            ...
            /* ==========================
               INSERIMENTO MANUALE - DENOMINAZIONE
            ========================== */
            ...
            /* ==========================
               INSERIMENTO MANUALE - CODICE ADM
            ========================== */
            ...
            document.addEventListener('click', e => {
                const btn = e.target.closest('button[data-ean]');
                if (!btn) return;

                delete scannedItems[btn.dataset.ean];
                renderTable();
            });

            /* ==========================
               SUBMIT → SERIALIZZAZIONE
            ========================== */
    ...
    <script>
        /* document.addEventListener('DOMContentLoaded', () => {
            ...
            // 🔁 Autocomplete denominazione → codice + tipo + prezzo
            denominazioneInput.addEventListener('input', () => {
                const val = denominazioneInput.value.trim();
                const options = document.querySelectorAll('#denominazioni option');
                let found = false;

                options.forEach(opt => {
                    if (opt.value === val) {
                        codiceInput.value = opt.dataset.codice || '';
                        tipoConfezioneInput.value = opt.dataset.tipo || '';
                        prezzoConfezioneInput.value = opt.dataset.prezzo || '';
                        denominazioneInput.value = opt.dataset.denominazione || '';
                        found = true;
                    }
                });
            ...
            // 🔁 Autocomplete codice → denominazione + tipo + prezzo
            codiceInput.addEventListener('input', () => {
                const opt = document.querySelector(`#codici option[value="${codiceInput.value}"]`);
                if (opt) {
                    denominazioneInput.value = `${opt.dataset.denominazione} (${opt.dataset.tipo})`;
                    tipoConfezioneInput.value = opt.dataset.tipo || '';
                    prezzoConfezioneInput.value = opt.dataset.prezzo || '';
                } else {
                    denominazioneInput.value = '';
            ...
            // 📦 Scansione barcode → autocompleta + focus su quantità + default = 1
            eanInput.addEventListener('keydown', e => {
                if (e.key !== 'Enter') return;
                e.preventDefault();
                ...
                fetch(`/products/by-barcode/${ean}`)
                    ...
                    .then(data => {
                        ...
                        denominazioneInput.value = `${data.denominazione} (${data.tipo_confezione})`;
                        tipoConfezioneInput.value = data.tipo_confezione || '';
                        prezzoConfezioneInput.value = data.prezzo_confezione_euro || '';
            ...
            // 🔄 Pulisce parentesi prima del submit
